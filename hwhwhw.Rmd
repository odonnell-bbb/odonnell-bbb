...
#1.Build a tuning procedure for the hyperparameter of K using "tidymodels." 
#Please make sure you include all the following steps: 
...
```{r}
churn.df <- read.csv("Churn.csv", header=TRUE)
```

```{r}
churn.df<-churn.df%>%select(gender,SeniorCitizen,Partner,Dependents,tenure,MonthlyCharges,Contract,Churn)

churn.df<-drop_na(churn.df)

churn.df$gender<-as.factor(churn.df$gender)
churn.df$Dependents<-as.factor(churn.df$Dependents)
churn.df$Partner<-as.factor(churn.df$Partner)
churn.df$Contract<-as.factor(churn.df$Contract)
...
```{r}
set.seed(123)
churn.split<-initial_split(churn.df, prop=.7, strata = Churn)
...
```{r}
set.seed(123)
model_rec<-recipe(Churn~., churn.train) %>%
...
```{r RECODE: Churn, include=TRUE}
levels(subset1$churn %>% as.factor())
```
...
```{r, include=TRUE, message=FALSE, results='hide'}
subset1<-subset1 %>% mutate(churn=fct_rev(churn))
levels(subset1$churn)
```
```{r, include=TRUE, message=FALSE}
round(prop.table(table(subset1$churn)) * 100, digits = 1)
```
...
```
# Accuracy and roc_auc show a clear pattern when K increases these metrics also increase. If the best K was based solely on accuracy and roc_auc I would pick the highest point which is k=14 or k=15.
...
```{r}
model14<-nearest_neighbor(neighbors=14) %>% 
  set_mode("classification") %>% 
  set_engine("kknn")
...
```{r}
last_fit(best_model, subset.split) %>% collect_metrics()
```
